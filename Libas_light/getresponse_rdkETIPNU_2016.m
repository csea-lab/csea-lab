...
function [conditionvecPicRT, condiPNU, accuracy] = getresponse_rdkETIPNU_2016(filepath);
...
a = 1;
 ...
 while a > 0
     ...
     %added this to account for odd .dat files with tabs instead of spaces
     if sum(blankindices)==0
         blankindices = find(a==[sprintf('\t')]); 
     else
     end
     ...
     if rt(trialcount) < 0, rt(trialcount) = rt(trialcount)+10000; end %should be 8650?? and add the 4 retraces too 66.6ms    
...
accuracy.p = ((corr_p + numcr_p) / (total_p + totalfa_p)).*100
accuracy.n = ((corr_n + numcr_n) / (total_n + totalfa_n)).*100
accuracy.u = ((corr_u + numcr_u) / (total_u + totalfa_u)).*100
...
eval(['save ' filepath '.con3.dat condiPNU -ascii'])
