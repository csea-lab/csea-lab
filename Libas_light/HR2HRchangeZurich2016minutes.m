% ECG2HRchange
function [CON_BPMmat, BPMavgmat8cond_bsl] = HR2HRchangeZurich2016minutes(csvfiledata, csvfilejournal)
...
while line ~= -1; 
    ...
    % journalmat(index, 1) = str2num(line(commaindices(3)+1:commaindices(4)-1))             
      journalmat(index, 1) = str2num(line(11:15)).*60
    ...
    conditionword = line(commaindices(1)+17:apoindices(end)-1); disp('label word:'), disp(conditionword(1:end-1))
...
for trial = 2: size(journalmat,1) 
    ...
    trialonsetindex = find(abs(m(:,2) - journalmat(trial,1)) == (min(abs(m(:,2) - journalmat(trial,1))))) % find the start time point of each trial in the data file, m
        ...
        elseif trialonsetindex + 11 > size(m,1) 
        BPMmat = [BPMmat; ones(1,9).*60]; 

        else


        Rwavestamps = m(trialonsetindex-2:trialonsetindex+11, 2)-(journalmat(trial,1)-2)
