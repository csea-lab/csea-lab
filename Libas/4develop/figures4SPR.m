...
for k = 1:nSeries
    white          = randn(N,1);          % zero‑mean white noise
    brownSeries(:,k) = cumsum(white);    % Brownian (1/f) noise
    brownSeries(:,k) = brownSeries(:,k) + 5* sin(2*pi*(10+(randn(1,1)*1.5))*time);
end
...
% ---------- Average spectrum and convert to dB ----------
meanPowerLinear = mean(powerSpec,2);            % average in linear domain
logPowerMean    = movmean(10*log10(meanPowerLinear), 6);   % dB scale
...
% ---------- Average spectrum and convert to dB ----------
meanPowerLinear = mean(powerSpec,2);            % average in linear domain
logPowerMean    = movmean(10*log10(meanPowerLinear), 3);   % dB scale
