...
conditionvec = table2array(conditionvec); 
     ...
     for index1 = 1:length(EEG.event)
        ...
        for index2 = 1:length(newtrigvec)
            ...
            EEG_temp.event(index2).latency = newtrigvec(index2);
            EEG_temp.event(index2).urevent = index2;
            EEG_temp.urevent(index2).type = conditionvec(index2);
            EEG_temp.urevent(index2).latency = newtrigvec(index2);
        end
