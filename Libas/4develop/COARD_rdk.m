...
s3 = serial('/dev/ttyUSB0', 'BaudRate', 4800, 'DataBits', 8, 'StopBits', 1, 'Parity', 'none');
fopen(s3);


% files
datafilename = strcat('COARD_rdk_',num2str(subNo),'.dat'); % name of data file to write to
...
try
    ...
    convector = tempvector(newindices);
    ...
    for trial = 1:ntrials
        ...
        % read in real picture
        imdatatemp=imread([stimdir filemat_perm(trial).name]); %reads in random picture for every trial but records pic used
        imdata = double(imresize(imdatatemp, [800 800]));
        imdata(:, :, 1) =  imdata(:, :, 1) .* apert; 
        imdata(:, :, 2) =  imdata(:, :, 2) .* apert; 
        imdata(:, :, 3) =  imdata(:, :, 3) .* apert; 

        % make texture of real picture
        ...
        if convector(trial) == 1 % it is not a target
            ...
            for flickindex = 1:75
                ...
                if flickindex < 26
                    ...
                    Screen('DrawTexture', w, tex, [], CenterRect(tRect, wRect)); %shows pic
                end
