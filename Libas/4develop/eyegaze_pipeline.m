...
datavec = datamat.Samples.posX;

% % plot pupil vs trials: 
figure
plot(datamat.Samples.time, datamat.Samples.posX)
hold on
plot(datamat.Samples.time, datamat.Samples.posY)
...
% pick up the surrounding points after filtering
 ...
 rawfilt = filtfilt(filta, filtb, abs(diff(datamat.Samples.pupilSize)));
...
numcond = length(unique(convec));
 ...
 % 
  ...
  matoutbsl = bslcorr(matout', 1:100)'; 
  end
