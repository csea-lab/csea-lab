...
% sarah saved as ress_gaborgentone.csv
...
BFmap_RESS_select_csminus(31) %1.273 still small
...
%
figure
plot(faxisFFT(1:50),GS1(70, 1:50), LineWidth=2),hold on,
plot(faxisFFT(1:50),GS2(70, 1:50), LineWidth=2),
plot(faxisFFT(1:50),GS3(70, 1:50), LineWidth=2),
...
gm22 = getfilesindir(pwd, 'GM22.at2*');
...
alphagm21 = mean(GM22pow3_21_bsl(70,800:1300,8),2);
alphagm22 = mean(GM22pow3_22_bsl(70,800:1300,8),2);
alphagm23 = mean(GM22pow3_23_bsl(70,800:1300,8),2);
alphagm24 = mean(GM22pow3_24_bsl(70,800:1300,8), 2);
alpha_gm = [alphagm21; alphagm22; alphagm23; alphagm24];

% Plot the points and connect them with lines
figure; % Open a new figure window
plot(alpha_gm', '-', 'LineWidth', 1); % '-o' adds markers and connects them with lines
xlabel('Condition');
ylabel('Alpha Grand Mean Values');
legend


%plot alpha differences by condition over time
figure
plot(taxis(300:1300), squeeze(GM22pow3_21_bsl(70, 300:1300, 8)))
hold on
plot(taxis(300:1300), squeeze(GM22pow3_22_bsl(70, 300:1300, 8)))
plot(taxis(300:1300), squeeze(GM22pow3_23_bsl(70, 300:1300, 8)))
plot(taxis(300:1300), squeeze(GM22pow3_24_bsl(70, 300:1300, 8)))
...
for elec = 1:size(repeatmat_alpha,1)
    for timepoint = 1:size(repeatmat_alpha,2) %180 timepoints, decimated earlier
        for  frequency = 1:size(repeatmat_alpha,3) %25 frequencies, decimated earlier (8th is alpha)
            BFmap_alpha_select(elec, timepoint, frequency) = bootstrap2BF_z(squeeze(selectBootstrap(elec,timepoint, frequency, :)),squeeze(selectBootstrapPerm_alpha(elec,timepoint, frequency, :)), 0);
        end
    end
    disp(['elec', num2str(elec)])
end

%% read in at files that andreas sent
linear = ReadAvgFile('Log10TypicalLinearBFs.at');
antilinear = ReadAvgFile('Log10AntiLinearBFs.at');
allnothing = ReadAvgFile('Log10allnothingBFs.at');

early_bfmap_linear = squeeze(mean(antilinear (75, 30:63), 2))
middle_bfmap_linear = squeeze(mean(antilinear(75, 64:97), 2))
late_bfmap_linear = squeeze(mean(antilinear  (75, 98:130), 2))
% antilinear  : [129 × 180] matrix of log10 Bayes factors
%               (129 electrodes, 180 decimated time-samples)

% Channel 75, three time windows
early_bfmap_linear  = mean(antilinear(70, 30:63))  % 0–600 ms
middle_bfmap_linear = mean(antilinear(70, 64:97))   % 600–1200 ms
late_bfmap_linear   = mean(antilinear(70, 98:130))  % 1200–2000 ms
