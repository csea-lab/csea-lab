...
% conditions2select is a cell array with condition names that we want e.g.
% {  '21' '22' '23' '24' }; {'all'} will select every condition found from the convecfun
...
for con_index = 1:size(conditions2select,2)
    ...
    %skip if condition is not in event
    if sum(ismember({EEG_allcond.event.type}, conditions2select{con_index})) == 0
        disp(['condition ' conditions2select{con_index} ' removed or not presented'])
        continue
    end
