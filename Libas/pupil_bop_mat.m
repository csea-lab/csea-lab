function [matcorr, pupilbycond, percentbadvec] = pupil_bop_mat(edffile, datfile)
%gets cue indexes 
datamat = Edf2Mat(edffile);

temp = ismember(datamat.Events.Messages.info,'cue_on');
cue_indx = find(temp);
...
%epoching 
taxis = -4000:2:2000;
mat = zeros(3, 3001, length(event_sample));

for x = 1:size(event_sample,1)
pupilmat(:, x) = pupil(event_sample(x)-2000:event_sample(x)+1000);  
horzmat(:, x) = horz(event_sample(x)-2000:event_sample(x)+1000);  
vertmat(:, x) = vert(event_sample(x)-2000:event_sample(x)+1000);  
end
...
for x = 1:size(event_sample,1)
percentbadvec(x)= (sum(isnan(matcorr(2,event_sample(x)-500:event_sample(x)+1500)))./length(mat(1,:,1))).*100; 
end
...
matcorr3d = zeros(3, 3001, 130);
for x = 1:size(event_sample,1)
matcorr3d(:, :, x) = matcorr(2:4,event_sample(x)-2000:event_sample(x)+1000);  end


% plot the data
figure
for x = 1:size(event_sample,1)
plot(taxis, matcorr3d(:,:,x)'), title (['CORRECTED - trial number:' num2str(x)]), pause(.5)
end
