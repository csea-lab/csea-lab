function [trialamp,winmat3d,phasestabmat,trialSNR] = freqtag_slidewinMovie(data, plotflag, bslvec, ssvepvec, foi, sampnew, fsamp, outname)
...
% ssVEP signal-to-noise ratio in decibels at the frequency of interest for each trial (trialamp) 
 ...
 samp=1000/sampnew;
  ...
  for trial = 1:NTrials
        ...
        frames(length(winshiftvec)) = struct('cdata', [], 'colormap', []);
...
%===========================================================
     ...
     for winshiftstep = 1:length(winshiftvec)
       ...
       if plotflag
           ...
           subplot(2,1,2), plot(1:sampcycle:shiftcycle*sampcycle, winmatsum'), title(['sum of sliding windows; number of shifts:' num2str(winshiftstep) ]), ylabel('microvolts'), axis([1 shiftcycle*sampcycle -100 100])
           
            % Capture the frame for the movie
            drawnow;  % Ensure plot updates before capturing frame
            frames(winshiftstep) = getframe(gcf);
...
v = VideoWriter('slidewinmovie', 'MPEG-4');  % Save as MP4
