function ft_taggingfeedback6(cfg, targfreq, distfreq)
...
% set up the lowpass filter at 40 Hz and highpass at 5 Hz
  ...
  [Blow,Alow] = butter(3, 50/(hdr.Fs/2)); 
...
while true
  ...
  if newsamples>=blocksize
   ...
   % data.trial{1} = ft_preproc_baselinecorrect(data.trial{1});
    ...
    figure(5)
    plot ([datold data.trial{1}']), axis([0 500 -.15 .15])
 ...
 size( [datold data.trial{1}'])
   ...
   % summand = ((pow(1,targindex)-pow(1, distindex))./(pow(1, targindex)+pow(1, distindex)).*100);
    summand = (mean(pow(1,distindex))./mean(pow(1, targindex))).*50
    ...
    subplot(3,1,1), plot(freqs, pow); 
    axis([cfg.foilim(1) cfg.foilim(2) 0 0.02]);grid
    subplot(3,1,2), plot(freqs, phase); 
    axis([cfg.foilim(1) cfg.foilim(2) -3.5 3.5]);
    subplot(3,1,3), plot(freqs, pow./(sum(pow))); 
    axis([cfg.foilim(1) cfg.foilim(2) 0 .05]);
