...
% set up the lowpass filter at 40 Hz and highpass at 5 Hz
  ...
  [Bhigh,Ahigh] = butter(4, 9/(hdr.Fs/2), 'high'); 
...
while true 
  ...
  if newsamples>=blocksize
   ...
   % data.trial{1} = ft_preproc_baselinecorrect(data.trial{1});
    ...
    figure(5)
    plot (data.trial{count}), title('incoming raw data')
 ...
 % data.fsample
    ...
    summand = mean(((pow(1,targindex)- pow(1, distindex)))).*20; %./(pow(1, targindex)+pow(1, distindex)).*100)); 
    summandappend = [summandappend summand]; 
    figure(10), plot(summandappend), title('difference measure')
    taxis = 0:0.001:1; 
    sound(double((sin((800-summand).*pi .*taxis)))) 
