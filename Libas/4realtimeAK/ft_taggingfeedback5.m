function ft_taggingfeedback5(cfg, targfreq, distfreq)
...
% set up the lowpass filter at 40 Hz and highpass at 5 Hz
  ...
  [Blow,Alow] = butter(2, 30/(hdr.Fs/2)); 
    
  [Bhigh,Ahigh] = butter(2, 4/(hdr.Fs/2), 'high'); 
...
while true
 ...
 size( [datold data.trial{1}'])
    ...
    [pow, freqs] = FFT_spectrum([datold data.trial{1}'], data.fsample);
   
    %size(pow)
   
    pow = mean(pow([12:14,16],:),1); 
    size(pow)
