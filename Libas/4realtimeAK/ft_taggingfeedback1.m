function ft_taggingfeedback1(cfg, targfreq, distfreq)
...
while true
  ...
  if newsamples>=blocksize
    ...
    targindex = find(round(est.Frequencies) == targfreq);
    distindex = find(round(est.Frequencies) == distfreq);
    

    summand = ((pow(1,targindex)-pow(1, distindex))./(pow(1, targindex)+pow(1, distindex)).*100); 
    taxis = 0:0.001:.5;
    sound(double((sin((150+summand) .*2.5 .*pi .*taxis))), 10000)
