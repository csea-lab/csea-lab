...
% Last Modified by GUIDE v2.5 21-Feb-2012 12:10:16
...
% --- Executes during object creation, after setting all properties.
function beginAxis_CreateFcn(hObject, eventdata, handles)
% hObject    handle to baseline (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in baseline.
function beginAxis_Callback(hObject, eventdata, handles)
% hObject    handle to baseline (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
beginAxis = round(str2num(get(handles.beginAxis, 'string')));
endAxis = round(str2num(get(handles.endAxis, 'string')));
figure
...
contourf(WaData.tAxis,WaData.fAxis,squeeze(WaData.WaPower(sensorNum,:,:))');caxis([beginAxis endAxis]),colorbar
else
contourf(WaData.tAxis,WaData.fAxis,squeeze(mean(WaData.WaPower(sensorNum,:,:),1))');caxis([beginAxis endAxis]),colorbar
end

elseif bsc ==1
    
if length(sensorNum) == 1 
contourf(WaData.tAxis,WaData.fAxis,squeeze(bslcorrWAMat_as(WaData.WaPower(sensorNum,:,:), [startBase:endBase]))');caxis([beginAxis endAxis]),colorbar
else
% contourf(WaData.tAxis,WaData.fAxis,squeeze(mean(bslcorrWAMat_as(WaData.WaPower(sensorNum,:,:), [startBase:endBase])),1)');colorbar  
contourf(WaData.tAxis,WaData.fAxis,squeeze(bslcorrWAMat_as(mean(WaData.WaPower(sensorNum,:,:),1) , [startBase:endBase]))');caxis([beginAxis endAxis]),colorbar  %%Does this work? Baseline correcting a mean...
...
% hObject    handle to beginAxis (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of beginAxis as text
%        str2double(get(hObject,'String')) returns contents of beginAxis as a double
