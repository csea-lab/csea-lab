function [HRmat] = getHRfromICA_interactive(filemat)
%
plotflag = 1; 
srate = 500; 
...
for fileindex = 1:size(filemat,1)
    ...
    yesno = input('keepityesno?    ');

    
    if yesno == 0 % if there is no ECG
        HRmat(trial,:) = ones(1,13).*60; 
    else
