...
% 3) determine the error for experience
...
Y = labelvectorfull(randperm(length(labelvectorfull)))';
...
for timepoint1 = 1:size(mat3d3avg,2)
    ...
    for timepoint2 = 1:size(mat3d3avg,2)
         ...
         error(freq, timepoint1, timepoint2) = sum(diag(aha))./(sum(sum(aha))); 
...
%plot the result
for x = 1:freq
subplot(5,3,x),  imagesc(squeeze(error(x,:,:))), caxis([.4 0.7]), title(x), colorbar, end
