...
for fileindex = 1:size(filemat,1)
    ...
    % load each person's data
        ...
        groupmatpredicted = [groupmatpredicted; temp]; 
        ...
        BICmat(fileindex, model) = length(R) .* log(RSS./length(R)) + (length(BETA)+2)* log(length(R))
        ...
        groupmatdata = [groupmatdata; ratings./max(ratings)]; 
