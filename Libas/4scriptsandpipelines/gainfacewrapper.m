...
% now run appropriate field in palamedesgainesface for this condition
%%
%%%% 5. MAIN EFFECT of threat in 6 conditions, with 18 people 108 files (18 * 6) 
% get filemat of all hamp files, then do ROI
clear
filemat = getfilesindir(pwd, '*hamp12');
...
% average like conditions, here for threat
mat4fitnew = zeros(2*18,13); 
mat4fitnew(1:2:end,:) = (mat4fit(1:6:end,:) + mat4fit(2:6:end,:)+ mat4fit(3:6:end,:))./3;
mat4fitnew(2:2:end,:) = (mat4fit(4:6:end,:) + mat4fit(5:6:end,:)+ mat4fit(6:6:end,:))./3;
...
%%
%%%% 6. MAIN EFFECT of threat in faces in 4 conditions, with 18 people 72 files (18 * 4) 
% get filemat of all hamp files, then do ROI
clear
filemat = getfilesindir(pwd, '*hamp12');
...
% average like conditions, here for threat
mat4fitnew = zeros(2*18,13); 
mat4fitnew(1:2:end,:) = (mat4fit(1:4:end,:) + mat4fit(2:4:end,:))./2;
mat4fitnew(2:2:end,:) = (mat4fit(3:4:end,:) + mat4fit(4:4:end,:))./2;
...
%%
%%%% 7. MAIN EFFECT of expression in faces in 4 conditions, with 18 people 72 files (18 * 4) 
% get filemat of all hamp files, then do ROI
clear
filemat = getfilesindir(pwd, '*hamp12');
...
% average like conditions, here for expression
mat4fitnew = zeros(2*18,13); 
mat4fitnew(1:2:end,:) = (mat4fit(1:4:end,:) + mat4fit(3:4:end,:))./2;
mat4fitnew(2:2:end,:) = (mat4fit(2:4:end,:) + mat4fit(4:4:end,:))./2;
...
% average like conditions, here for content
mat4fitnew = zeros(4*18,13); 
mat4fitnew(1:4:end,:) = (mat4fit(1:8:end,:) + mat4fit(5:8:end,:))./2;
mat4fitnew(2:4:end,:) = (mat4fit(2:8:end,:) + mat4fit(6:8:end,:))./2;  
mat4fitnew(3:4:end,:) = (mat4fit(3:8:end,:) + mat4fit(7:8:end,:))./2; 
mat4fitnew(4:4:end,:) = (mat4fit(4:8:end,:) + mat4fit(8:8:end,:))./2; 

% make a cell array for palamedesgainface script, then do normalization of each person from 0 to 1
cellmat4fit = []; 
for x = 1:18, cellmat4fit{x} = mat4fitnew(x*4-3:x*4,:), end
for x = 1:18, cellmat4fit{x} = (cellmat4fit{x}+abs(min(min(cellmat4fit{x})))),  end
for x = 1:18, cellmat4fit{x} = (cellmat4fit{x}./(max(max(cellmat4fit{x})))),  end

% now run appropriate field in palamedesgainesface for this condition
%%
%%%% 9. MAIN EFFECTS of pictype in 6 conditions, with 18 people 198 files (18 * 6) 
% get filemat of all hamp files, then do ROI
clear
filemat = getfilesindir(pwd, '*hamp12');
mat4fitnew = []; 
...
% average like conditions, here for pictype
mat4fitnew = zeros(3*18,13); 
mat4fitnew(1:3:end,:) = (mat4fit(1:6:end,:) + mat4fit(4:6:end,:))./2;
mat4fitnew(2:3:end,:) = (mat4fit(2:6:end,:) + mat4fit(5:6:end,:))./2;  
mat4fitnew(3:3:end,:) = (mat4fit(3:6:end,:) + mat4fit(6:6:end,:))./2;

% make a cell array for palamedesgainface script, then do normalization of each person from 0 to 1
cellmat4fit = []; 
for x = 1:18, cellmat4fit{x} = mat4fitnew(x*3-2:x*3,:); end
for x = 1:18, cellmat4fit{x} = (cellmat4fit{x}+abs(min(min(cellmat4fit{x})))),  end
for x = 1:18, cellmat4fit{x} = (cellmat4fit{x}./(max(max(cellmat4fit{x})))),  end
% now run appropriate field in palamedesgainesface for this condition
