function [] = modelfit_alpha_PH_bop_Tcourse(appmatfilemat, igfilemat)
...
%          if timepoint/50 == round(timepoint/50), disp(timepoint), end
        ...
        for channel = 1:129            
            ...
            [BETA,R,J,COVB,MSE(fileindex,model)] = nlinfit(contingencies, power(channel,:)',@PearceHallModIntcpt, [0.5 0.5 .1]);
        ...
        BICmat(channel, timepoint) = length(R) .* log(RSS./length(R)) + (length(BETA)+2)* log(length(R));
