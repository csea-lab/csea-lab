function BrandonTriggertest_final(subNo)
...
if subNo < 9999 && fopen(datafilename, 'rt') ~= -1
    fclose('all');
...
try
    ...
    % stimulus folder (PC)
    Sstimdir = '/home/gibbgrad/Desktop/SSVEP_Change_Detection_Task_MIMIC3/15Hz_Face_Facebor/sad/';
    Hstimdir = '/home/gibbgrad/Desktop/SSVEP_Change_Detection_Task_MIMIC3/15Hz_Face_Facebor/happy/';
    Astimdir = '/home/gibbgrad/Desktop/SSVEP_Change_Detection_Task_MIMIC3/15Hz_Face_Facebor/angry/';

    % read stimuli
    filemat_spics = dir([Sstimdir 'S*.jpg']); % file type should be in 'capital'
    filemat_hpics = dir([Hstimdir 'S*.jpg']);
    filemat_apics = dir([Astimdir 'S*.jpg']);

    filemat_spics = [filemat_spics; filemat_spics]; 
    filemat_hpics = [filemat_hpics; filemat_hpics]; 
    filemat_apics = [filemat_apics; filemat_apics]; 
    
    %     here : add mouse click to start
    %     instruction screen
          imdata_instructions=imread('/home/gibbgrad/Desktop/SSVEP_Change_Detection_Task_MIMIC3/15Hz_Face_Facebor/Instructions.JPG');
   ...
   %sending out markers for each condition
    ...
    %     here : add mouse click to start
    %     instruction screen
          imdata_instructions=imread(['/home/gibbgrad/Desktop/SSVEP_Change_Detection_Task_MIMIC3/15Hz_Face_Facebor/' restingconditions{r}]);
          tex_instructions=Screen('MakeTexture', w, imdata_instructions);
          tRect_1=Screen('Rect', tex_instructions);
    % draw texture to backbuffer
          Screen('DrawTexture', w, tex_instructions, [], CenterRect(tRect_1, wRect));
   ...
   % putvalue(uddobj,[0 0 0 0 0 0 0 0],1:8);% set parallel back to zero
    ...
    %     here : add mouse click to start
    %     instruction screen
          imdata_instructions=imread('/home/gibbgrad/Desktop/SSVEP_Change_Detection_Task_MIMIC3/15Hz_Face_Facebor/Instructions1.JPG');
          tex_instructions=Screen('MakeTexture', w, imdata_instructions);
          tRect_1=Screen('Rect', tex_instructions);
         % draw texture to backbuffer
          Screen('DrawTexture', w, tex_instructions, [], CenterRect(tRect_1, wRect));
    ...
    for trial = 1:ntrials
        ...
        elseif convector(trial) == 2, conlabel='angry'; imdata_angry = imread([Astimdir filemat_apics(angryindex).name]); tex_face = Screen('MakeTexture', w, imdata_angry); tRect_face = Screen('Rect', tex_face);
        elseif convector(trial) == 3, conlabel='sad'; imdata_sad = imread([Sstimdir filemat_spics(sadindex).name]); tex_face = Screen('MakeTexture', w, imdata_sad); tRect_face = Screen('Rect', tex_face);
        end
tic
        ...
        if convector(trial) == 1, hapindex = hapindex + 1; angryindex = angryindex; sadindex = sadindex;
        elseif convector(trial) == 2, hapindex = hapindex; angryindex = angryindex+1; sadindex = sadindex;
        elseif convector(trial) == 3, hapindex = hapindex; angryindex = angryindex; sadindex = sadindex+1;
        end
 Screen('DrawText', w, 'Was there a change? Yes = Left click, No = Right Click', 1920/3, 1080/2, 255); % change 
 Screen('Flip', w); % show text
