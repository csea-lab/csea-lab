function [matcorr, pupilbycond, percentbadvec] = natsounds_pupil(ascfile, datfile); 
%post processing: 
...
taxis = -1000:2:3000;
mat = zeros(3, 2001, 350);
for x = 1:size(indices,1)
mat(:, :, x) = asc.dat(2:4,indices(x)-500:indices(x)+1500);  
end
...
matcorr = zeros(3, 2001, 350);
for x = 1:size(indices,1)
matcorr(:, :, x) = ascorr.dat(2:4,indices(x)-500:indices(x)+1500);  end
...
for condi = 1:10
    hold on,  
        if condi == CSplusloc, plot(taxis, squeeze(mean(matcorr(3,:,conditions==condivec(condi)), 3)), '--ks', 'LineWidth',2)
        elseif condi == CSplusloc+5, plot(taxis, squeeze(mean(matcorr(3,:,conditions==condivec(condi)), 3)), '--rs', 'LineWidth',2)
        else
        plot(taxis, squeeze(mean(matcorr(3,:,conditions==condivec(condi)), 3)))
        end
end
