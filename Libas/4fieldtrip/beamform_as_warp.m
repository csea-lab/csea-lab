function [source,sensFid,norm_elec, segmri] = beamform_as_warp(cfgas, filemat, plotflag)
...
% filemat: matrix with filenames taht lead to mat files, which should contain a strcuture array "freq"
...
elseif hasmri & ~hassegmented
    ...
    [segmri] = ft_volumesegment_as(cfg, cfgas.mri, cfgas.path);
...
%%%%%%%%%%%%%%%
    ...
    [vol,cfg]=ft_prepare_singleshell(cfg,segmri);
...
% %%%%%%%if a default mesh is needed, that covers more than just the brain
% and does not skip white matter and csf, but covers everything as the
% source space: 
%
% cfg = [];
% cfg.seg.resolution = .5
% cfg.tightgrid = 'yes';
% cfg.inwardshift = -1.5;
% template_grid = prepare_dipole_grid(cfg, vol, sensFid);
...
cfg.mri = segmri;
%cfg.spheremesh = 8000; 
template_grid = ft_prepare_dipole_grid(cfg, vol, sensFid);
...
if cfgas.warpflag
    ...
    [norm_elec] = ft_electroderealign(cfg);
...
disp(cfgas.method)
 ...
 % save the source file
     if isstruct(filemat)
     eval(['save ' deblank(filemat(index).name) '.source.mat source -mat']) 
     else
     eval(['save ' deblank(filemat(index,:)) '.source.mat source -mat'])
     end
