...
def create_dictionary_of_old_and_new_paths(input_dir: Path, bids_dir: Path) -> dict:
    ...
    for old_path in old_paths:
        ...
        if filetype_of(old_path) == "anat":
            ...
            new_path = bids_dir / f"sub-{subject_id_of(old_path)}" / "func" / f"sub-{subject_id_of(old_path)}_task-bop_bold{old_path.suffix}"

        elif filetype_of(old_path) == "eeg":
            new_path = bids_dir / f"sub-{subject_id_of(old_path)}" / "eeg" / f"sub-{subject_id_of(old_path)}_task-bop_eeg{old_path.suffix}"
